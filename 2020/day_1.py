...
# Print the product of any two numbers that add up to 2020
def product_of_two(expenses):
    ...
    for i in range(length):
        ...
        index = i
        for v in expenses:
            if v + (expenses[index]) == 2020:
                result = v * (expenses[index])
                print("result is " + str(result))
    pass

# Print the product of any three numbers that add up to 2020
def product_of_three(expenses):
    # Getting length of list
    length = len(expenses)
    for i in range(length):
        result = 0
